 ...
 */
@Controller
@RequestMapping(value = "/statisticsTdghFristFaildAction")
public class FirstMailFaildAction extends StatisticsAction {
	@Resource(name = "statisticsMailFaildStrategyImpl")
	protected IStatisticsFaildFacade statisticsMailFaildStrategyImpl;

	/**
	 * 首次未妥投信息统计 需要的数据仅为未妥投，投递并签收且未妥投应该剔除
	 * 
	 * @param request
	 * @param response
	 * @return
	...
	public ModelAndView getStatistics(HttpServletRequest request,
		...
		List<Object[]> list =statisticsMailFaildStrategyImpl.getFaildList(statisticsMailFaildStrategyImpl.Statistics(map));
		request.setAttribute("firstmailfaild", list);
		ExportUtils.ExportTXT(list, request);
		ExportUtils.ExportEXCEL(list, request);
		return new ModelAndView("reportstatistics/firstmailfaildmain");
	}
